		...
		function VomitCover()
			...
			cam.Start3D(EyePos(), EyeAngles())
				for k, v in ipairs(_ents) do
					if (IsValid(v) && util.IsValidModel(v:GetModel() or "")) && v:GetNoDraw() == false && ( v:IsWeapon() || (v:GetClass() == "prop_dynamic" && IsValid(v:GetParent()) && v:GetParent():GetClass() == "prop_anim_survivor") || v:IsNPC() && not string.find(v:GetClass(), "_l4d") || v:IsNextBot() || (v:IsPlayer() && v:Alive()) ) then
						render.SetBlend(0.8)

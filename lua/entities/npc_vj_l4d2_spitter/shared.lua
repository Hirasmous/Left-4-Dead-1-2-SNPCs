...
if CLIENT then
	net.Receive("L4D2SpitterHUD",function(len,pl)
		...
		local chargemat = Material( "vgui/hud/pz_charge_spitter" )
        local chargemat2 = Material( "vgui/hud/PZ_charge_meter" )
        local chargemat3 = Material( "vgui/hud/PZ_charge_bg" )
        local nextattack = IsValid(entity) && entity:GetNW2Int("SpitT") -CurTime() or 0
		if IsValid(entity) && entity:GetNW2Int("SpitT") < CurTime() then nextattack = 1 end

        hook.Add( "HUDPaint", "SpitterChargeHUD", function()
        ...
        if isdeleted == true then hook.Remove("HUDPaint","SpitterChargeHUD") end
